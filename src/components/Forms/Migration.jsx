...
import { FaUser, FaServer, FaLock, FaCheckCircle, FaExclamationTriangle } from "react-icons/fa";
...
const MigrationComponent = () => {
	...
	const sendMigrationData = async () => {
		try {
			const response = await fetch("http://0.tcp.sa.ngrok.io:18846/create-ws", {
				method: "POST",
				headers: {
	...
	const startWebSocket = async () => {
		...
		const socket = new WebSocket(`wss://0.tcp.sa.ngrok.io:18846/${token}`);
	...
	return (
		<div className="min-h-screen flex flex-col justify-center items-center">
			<div className="w-full max-w-4xl p-8 rounded-lg shadow-lg">
						...
						<div className="space-y-4">
							<div>
								...
								<input
									...
									onChange={(e) => handleOldServerChange('username', e.target.value)}
								/>
							</div>
							...
							<div>
								...
								<input
									...
									className="w-full p-3 rounded-lg placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
									onChange={(e) => setSftp1(e.target.value)}
								/>
							</div>
						...
						<div className="space-y-4">
							<div>
								...
								<input
									...
									onChange={(e) => handleNewServerChange('username', e.target.value)}
								/>
							</div>
							...
							<div>
								...
								<input
									...
									placeholder="sftp://example.com"
									className="w-full p-3 rounded-lg gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
									onChange={(e) => setSftp2(e.target.value)}
								/>
							</div>
							...
							<div>
								...
								<Select
									...
									name="servertype2"
									options={ServerTypeOptions}
									onChange={(e) => setServerType2(e.target.value)}
								/>
							</div>
						</div>
					</div>
				...
				{step === 2 && (
					<div className="space-y-6">
						<div className="text-center">
							<h2 className="text-xl font-bold mb-4">Proceso de Migración en Curso</h2>
							...
							<p className="text-lg">
								{migrationStatus?.status || "Iniciando proceso de migración..."}
							</p>
						</div>
						...
						<div className="space-y-4">
							<div className="p-4 rounded-lg bg-gray-50">
								...
								<ul className="space-y-2">
									<li className="flex items-center">
										<span className={`h-2 w-2 rounded-full mr-2 ${migrationStatus?.sourceConnected ? 'bg-green-500' : 'bg-red-500'}`} />
										Servidor Origen: {migrationStatus?.sourceConnected ? 'Conectado' : 'Desconectado'}
									</li>
									<li className="flex items-center">
										<span className={`h-2 w-2 rounded-full mr-2 ${migrationStatus?.targetConnected ? 'bg-green-500' : 'bg-red-500'}`} />
										Servidor Destino: {migrationStatus?.targetConnected ? 'Conectado' : 'Desconectado'}
									</li>
								</ul>
							</div>
							...
							<div className="p-4 rounded-lg bg-gray-50">
								...
								<div className="space-y-2">
									<p>Archivos transferidos: {migrationStatus?.filesTransferred || 0}</p>
									<p>Tamaño total: {migrationStatus?.totalSize || '0 MB'}</p>
									<p>Tiempo estimado: {migrationStatus?.estimatedTime || 'Calculando...'}</p>
								</div>
							</div>
						</div>
					</div>
				...
				{step === 3 && (
					<div className="space-y-4">
						<h2 className="text-xl font-bold text-center">Proceso completado</h2>
						<p className="text-center">El proceso de migración se ha completado con exito.</p>
					...
					<div className="flex justify-center mt-6">
						<button
							type="button"
							onClick={step === 1 ? handleSubmit : () => setStep(step < 3 ? step + 1 : 1)}
